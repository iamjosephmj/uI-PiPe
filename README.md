...
- **Shared UID callers:** if a calling uid maps to more than one package (`android:sharedUserId`), the gate requires **every** package under that uid to be trusted by the authorizer before admitting — the safer default, since any one of those packages could be driving the call.
...
`sample-host`'s suite covers pane render, touch + IME interactivity, channel round-trips both ways, resize, and clean teardown, plus a security test proving the host denies an evil provider (no surface, no bind). `evil-host`'s suite proves the provider denies an evil host (with an allow-all host-side authorizer, so only the provider's gate is under test) even though the evil host's own authorization policy would have let it through.

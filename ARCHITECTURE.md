...
Nor is a host limited to a single provider. Each `PipeFullScreen.open` adds an independent child window to the host's window token, so a host can open several panes — from several processes — at once; with transparent panes they composite into one surface (`:sample-solo`'s "Open 3 panes" runs host + three `:pane*` processes = four processes, three provider windows in one host window). Because the panes are full-screen and touch-modal, only the top-most is the input target — independent *interactive* multi-pane would need non-overlapping touch regions or `FLAG_NOT_TOUCH_MODAL`; as a process/window model, though, N processes rendering into one window is unrestricted.

...
**Process priority.** A pane renders in the provider's process, so it's worth knowing where the platform ranks that process. By default (`PipeBindImportance.NORMAL`) the host binds the provider with `BIND_AUTO_CREATE` only, which raises the provider to **visible** importance (`oom_score_adj` ≈ 100) — alive and unthrottled while the pane is up, but ranked *below* the host, so under memory pressure the pane's process is reclaimed first and a provider can never make itself harder to kill than the host. When a host fully trusts a pane (typically its own same-app processes) it can pass `bindImportance = PipeBindImportance.IMPORTANT`, which adds `BIND_IMPORTANT` so the provider inherits the host's foreground/top importance (`oom_score_adj` ≈ 0) and sits co-equal with it. Measured on a Pixel 6 Pro with `:sample-solo`'s "Open 3 panes": `NORMAL` → host adj 0, three panes adj 100; `IMPORTANT` → all four processes adj 0. The default is deliberately the subordinate one — priority inflation is opt-in, not automatic.
